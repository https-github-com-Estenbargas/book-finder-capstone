...
export async function  getPartialBookByBookId(request: Request, response: Response) : Promise<Response> {
    try {
        const {bookId} = request.params;
        const mysqlResult = await selectPartialBookByBookId(bookId);
        const data = mysqlResult ?? null
        const status: Status = {status: 200, data, message: null}
        return response.json(status)
