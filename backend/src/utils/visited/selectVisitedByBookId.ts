...
export async function selectVisitedByBookId(bookId: string): Promise<Visited[]> {
    try {
        ...
        const mySqlQuery: string = "SELECT BIN_TO_UUID(visitedId) as visitedId, BIN_TO_UUID(visitedBookId) as visitedBookId, BIN_TO_UUID(visitedUserId) as visitedUserId FROM visited WHERE visitedBookId = UUID_TO_BIN(:bookId)";
